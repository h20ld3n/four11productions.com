...
(function($){
	...
	$.supersized = function( options ) {
		...
		if (options.start_slide){
			var activeslide = options.start_slide - 1;	//Default to defined start slide
		}else{
			var activeslide = Math.floor(Math.random()*options.slides.length);	//Generate random slide number
		}
		...
		//alert(options.slides.length);
		if (options.slides.length > 0){
			for(var i=0;i<options.slides.length;i++){
				if(i==0 && options.slides.length>1){element.append("<span class='leftnav'></span>");}
				if(i==0){var slideclass = "activeslide";}/* else if(i==1){var slideclass="nextslide";} */else if(i==(options.slides.length-1)){var slideclass="prevslide";}else{var slideclass = "";}
				//activeslide - 1 < 0  ? loadPrev = options.slides.length - 1 : loadPrev = activeslide - 1;	//If slide is 1, load last slide as previous
				//var imageLink = (options.slides[i].url) ? "href='" + options.slides[i].url + "'" : "";
				var imageLink =  "";
				//$("<img/>").attr("src", options.slides[i].image).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
				$("<img/>").attr("src", options.slides[i].image).appendTo(element).wrap('<a id="'+i+'" class="'+slideclass+'" ' + imageLink + linkTarget + '></a>');
				if(i==(options.slides.length-1) && options.slides.length>1){element.append("<span class='rightnav'></span>");}
			}
		}
		...
		$(window).load(function(){
			// change the play/pause buttons if autoplay option is enabled
			if (options.autoplay){			
			...
			//alert(options.display_info);
			if(options.display_info == 1){
				if (options.slide_captions) $('#slidecaption').html("<span class='showcontent active' id='"+activeslide+"'>info</span>");		//Pull caption from array
				//alert("show");
			}else{
				if (options.slide_captions) $('#slidecaption').html("<span class='showcontent' id='"+activeslide+"'>info</span>");
				//alert("hide");
			}
			...
			if (options.slideshow && options.slides.length > 1){				
				...
				//Thumbnail Navigation			
					...
					//Next thumbnail clicked
					$('#thumbnav .imgs').click(function() {
						if(!$(this).is(".curimg")){
							var imgidclicked= $(this).attr('id');						
							if(inAnimation) return false;		//Abort if currently animating
							
							clearInterval(slideshow_interval);	//Stop slideshow
							//alert(imgidclicked);
							nextclicked(imgidclicked);		//Go to next slide
							if(!(isPaused)) slideshow_interval = setInterval(nextslide, options.slide_interval);	//If not paused, resume slideshow					    
							return false; 
						}
				 ...
				 if (options.navigation){
				    ...
				    $(pauseplay).click(function() {
						...
						if (isPaused){
							...
							if ($(pauseplay).attr('class')=="pause") {$(pauseplay).removeClass("pause");$(pauseplay).addClass("play");}	//If image, swap to pause
			        		...
			        		if ($(pauseplay).attr('class')=="play") {$(pauseplay).removeClass("play");$(pauseplay).addClass("pause");}
			...
			$(document.documentElement).keyup(function (event) {
				...
				if ((event.keyCode == 37) || (event.keyCode == 40)) { //Left Arrow or Down Arrow
					...
					if (isPaused){
		        		...
		        		if ($(pauseplay).attr('class')=="play") {$(pauseplay).removeClass("play");$(pauseplay).addClass("pause");}	//If image, swap to play
		        		isPaused = true;	//Mark as paused
		       		}
		...
		if (options.slideshow && options.pause_hover){
			$(element).hover(function() {
				...
				if(!(isPaused) && options.navigation){
					if ($(pauseplay).attr('class')=="play") {$(pauseplay).removeClass("play");$(pauseplay).addClass("pause");}	//If image, swap to active
					slideshow_interval = setInterval(nextslide, options.slide_interval);
				}
		...
		//Next slide
		function nextslide() {
		//alert($(".content .eachposts").css("display"));
			if(inAnimation) return false;		//Abort if currently animating
				else inAnimation = true;		//Otherwise set animation marker 
		    
			//$(".content div.eachposts").hide();			
			...
			var nextslide = currentslide.next("a").length ? currentslide.next("a") : element.find('a:first');
			var prevslide = nextslide.prev("a").length ? nextslide.prev("a") : element.find('a:last');		
						
			//Update previous slide
			$('.prevslide').removeClass('prevslide');
			prevslide.addClass('prevslide');			
			//Get the slide number of new slide			
			parseInt(activeslide) + 1 == slides.length ? activeslide = 0 : activeslide++;
			...
			var curimg = parseInt(activeslide)+1;			
			$("#thumbnav a").removeClass("curimg");
		    $("#thumbnav a:nth-child("+curimg+")").addClass("curimg");
		    ...
		    if (options.slide_captions){
		    	(options.slides[activeslide].title) ? $('#slidecaption').html("<span class='showcontent' id='"+activeslide+"'>info</span>") : $('#slidecaption').html('');
		    }			
		...
		function prevslide() {
		    ...
		    var nextslide =  currentslide.prev("a").length ? currentslide.prev("a") : $(element).find('a:last');
			var prevslide =  nextslide.next("a").length ? nextslide.next("a") : $(element).find('a:first');
		...
		//Next slide
		function nextclicked(clickedimg) {			
			if(inAnimation) return false;		//Abort if currently animating
				else inAnimation = true;		//Otherwise set animation marker 
			
			//$(".content div.eachposts").hide();
			var curimg = parseInt(clickedimg) + 1;
			$("#thumbnav a").removeClass("curimg");
		    $("#thumbnav a:nth-child("+curimg+")").addClass("curimg");
		    var slides = options.slides;	//Pull in slides array
			...
			var nextslide = element.find("a#"+clickedimg); //the image to fedin
			var prevslide = currentslide;//the image to fedout 
			...
			//Update captions
		    if (options.slide_captions){			
		    	(options.slides[clickedimg].title) ? $('#slidecaption').html("<span class='showcontent' id='"+clickedimg+"'>info</span>") : $('#slidecaption').html('');
		    }			
		...
		$(".showcontent").live('click',function(){
			...
			$("div#post"+postnum).slideToggle("slow",function(){
				if (options.slideshow && options.slides.length > 1){
					if($("div#post"+postnum).css("display")=="block"){
						if(!(isPaused) && options.navigation){
							clearInterval(slideshow_interval);
						}							
					}else{	
		...
		if(options.display_info == 1){
			$(".content #post0").show();
			$("#slidecaption span.showcontent").click();
		}else{
			$(".content #post0").hide();
			alert("hide");
		}  */
			var showinfo = function(cid){
				//Show content inially 			
				$("div.eachposts").hide();			
				$("#post"+cid).show();
				$("#slidecaption span").addClass("active");			
			}		
	};
